...
app.get("/ratings", (req, res) => {
    executeQuery("SELECT nomeGiocatore as nome, timestamp, punteggio FROM partita ORDER BY punteggio DESC")
...
app.get("/questions",(req, resp) => {
    executeQuery("SELECT * FROM argomento INNER JOIN domanda ON argomento.titolo=domanda.argomento  WHERE titolo='informatica' ORDER BY RAND()")
    .then(response =>{
        const promise = [];
        const jsonFinal = {
...
const recuperaPunteggio = (answers) =>{
    return new Promise((resolve, reject)=>{
        ...
        answers.forEach(element=>{
                const queryRisultati = "SELECT punteggio FROM risposta where idDomanda="+element.id;
...
app.post("/answers", (req, resp) => {
    ...
    recuperaPunteggio(answers).then(response=> {
        const sqlAdd = "INSERT INTO partita(argomento,timestamp,punteggio,idGiocatore,nomeGiocatore) VALUES('informatica',"+new Date().getTime()+","+response+",1,'"+username+"')";
